...
template<>
inline xsd::Integer RegisteredDatatype<xsd::PositiveInteger, xsd_positiveInteger>::from_string(const std::string &s) {

    const std::regex positiveInteger_regex("[\\+]?[0-9]+");

    if (std::regex_match(s, positiveInteger_regex)) {
        xsd::PositiveInteger val =  std::stod(s);
