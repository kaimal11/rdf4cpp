...
template<>
inline xsd::NonNegativeInteger RegisteredDatatype<xsd::NonNegativeInteger, xsd_nonNegativeInteger>::from_string(const std::string &s) {

    const std::regex nnInteger_regex("[\\+]?[0-9]+");

    if (std::regex_match(s, nnInteger_regex)) {
        xsd::NonNegativeInteger val =  std::stod(s);
