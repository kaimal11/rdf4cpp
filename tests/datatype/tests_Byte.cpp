...
TEST_CASE("Datatype Byte") {
    ...
    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("139", type_iri), "XSD Parsing Error", std::runtime_error);

    CHECK_THROWS_WITH_AS(no_discard_dummy = Literal("-130", type_iri), "XSD Parsing Error", std::runtime_error);
